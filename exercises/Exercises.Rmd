...
Make sure that you downloaded and unzipped the course material availabe on [GitHub](https://github.com/crtahlin/AS2014_shiny_workshop.git) (at https://github.com/crtahlin/AS2014_shiny_workshop.git,  use the Download ZIP button on the right bottom of the page and unzip the file on your computer)
...
#### Exercise 1.0: draw the N(0,1) density function 
...
#### Exercise 1.1: allow the user to specify the mean value
...
#### Exercise 1.2: allow the user to specify the standard deviation
...
#### Exercise 1.3: add a vertical line at the mean value (if selected by the user)
...
Hint: use the `checkboxInput()` widget. Vertical lines in R can be drawn using the `abline()` function.

#### Exercise 1.4: add the possibility of choosing the color of the curve
...
`hr()` is an HTML builder function included in shiny; see the other builders typing `? builder` in R. 



#### Exercise 1.5: enable the user to specify the title of the plot and the label of the y-axis.
...
#### Exercise 1.6: display a table that reports (a subset of) the x and y values displayed in the graph.
...
#### Exercise 1.7: move the table in a separate tab. 
...
In shiny `tabsetPanel()` creates a tabset that contains `tabPanel()` elements. In this exercise the tabset will be included in the main panel. 


#### Exercise 1.8: deploy your app on the web
...
This can be obtained simply writing the name of the input slot within the rendering expression. 
...
#### Exercise 2.1: create an app that uploads the data from a cvs file and displays a table containing the data.
...
1. Organize the outputs in the main panel using tabsets. In the second tab display a graphical representation of the "ValuationInEuros variable". Here you can assume that the name of the variable is fixed: all the users will use exatly the same name in their data. 
...
Hints: The widgets in the user interface that depend on some of the other inputs (or outputs) must be defined in the server.R function, using the `renderUI()` function. Within the `renderUI()` function you can use the widgets that you are already familiar with (like `selectInput()`, etc). See the help of the function for details.
...
1. add an action button that will make sure that the code of the app is executed only when this button is pressed.
(reactivity will "wait" until the user clicks the button). 
...
Hint: you can use almost the same code used to download the data, only the contents of the `downloadHandle()` function (content slot) will be changed. In R we can prepare files containing graphs with commands like 
...
In this exercise we will explore the possibility of building an interactive document that embeds a shiny application (*shiny markdown* document). You might have noticed that this document (HTML) incuded some interctive features, which are not available if you open the document outside of the RStudio environment. 
...
A good place to get inspired about the existing widgets is the [widget gallery](http://shiny.rstudio.com/gallery/widget-gallery.html) or the tutorial [#3](http://shiny.rstudio.com/tutorial/lesson3/), both available on the shiny web page.
